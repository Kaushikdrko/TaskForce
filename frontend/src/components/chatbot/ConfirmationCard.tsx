import { CheckCircle2, Calendar, Trash2, Clock, CalendarDays, RefreshCw, Search, List, Settings } from 'lucide-react'
...
> = {
  ...
  suggest_schedule:     { icon: Clock,        label: 'Planning schedule',   color: 'text-amber-600',   bg: 'bg-amber-50',    border: 'border-amber-100' },
  search_items:         { icon: Search,       label: 'Searching',           color: 'text-blue-500',    bg: 'bg-blue-50',     border: 'border-blue-100' },
  list_all_tasks:       { icon: List,         label: 'Listing tasks',       color: 'text-slate-500',   bg: 'bg-slate-50',    border: 'border-slate-100' },
  get_user_preferences: { icon: Settings,     label: 'Loading preferences', color: 'text-slate-500',   bg: 'bg-slate-50',    border: 'border-slate-100' },
}
...
function getSubtitle(toolCall: ToolCall): string {
  ...
  if (tool === 'search_items' && args.query) return args.query as string
  if (tool === 'suggest_schedule' && args.items) return `${(args.items as unknown[]).length} items`
  return ''
}
...
export function ConfirmationCard({ toolCall }: { toolCall: ToolCall }) {
  ...
  if (toolCall.failed) return null
